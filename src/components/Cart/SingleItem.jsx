...
const SingleItem = ({ item, mobile = false }) => {
    var _a;
    ...
    if (mobile) {
        return (<div className="rounded-lg border border-gray-3 bg-white p-4">
        <div className="flex items-start gap-3.5">
          <div className="flex h-18 w-18 shrink-0 items-center justify-center rounded-[5px] bg-gray-2">
            <Image width={120} height={120} src={(_a = item.imgs) === null || _a === void 0 ? void 0 : _a.thumbnails[0]} alt="product"/>
          </div>

          <div className="min-w-0 flex-1">
            <h3 className="text-dark leading-6">{item.title}</h3>
            <p className="mt-1 text-custom-sm text-dark-4">₹{item.discountedPrice}</p>
          </div>

          <button onClick={() => handleRemoveFromCart()} aria-label="button for remove product from cart" className="flex h-9 w-9 items-center justify-center rounded-lg border border-gray-3 bg-gray-2 text-dark ease-out duration-200 hover:border-red-light-4 hover:bg-red-light-6 hover:text-red">
            <svg className="fill-current" width="20" height="20" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <div className="flex w-max items-center rounded-md border border-gray-3">
            <button onClick={() => handleDecreaseQuantity()} aria-label="button for remove product" className="flex h-10 w-10 items-center justify-center ease-out duration-200 hover:text-blue">
            ...
            <span className="flex h-10 w-12 items-center justify-center border-x border-gray-4">{quantity}</span>
            <button onClick={() => handleIncreaseQuantity()} aria-label="button for add product" className="flex h-10 w-10 items-center justify-center ease-out duration-200 hover:text-blue">
              <svg className="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <p className="font-medium text-dark">₹{item.discountedPrice * quantity}</p>
        </div>
      </div>);
    }
    return (<div className="flex items-center border-t border-gray-3 py-5 px-7.5">
      <div className="min-w-[400px]">
        <div className="flex items-center justify-between gap-5">
          <div className="w-full flex items-center gap-5.5">
            <div className="flex items-center justify-center rounded-[5px] bg-gray-2 max-w-[80px] w-full h-17.5">
              <Image width={200} height={200} src={(_a = item.imgs) === null || _a === void 0 ? void 0 : _a.thumbnails[0]} alt="product"/>
            </div>
