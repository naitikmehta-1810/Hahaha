...
const Analytics = () => {
    ...
    const metrics = useMemo(() => {
        ...
        const statusMap = products.reduce((acc, product) => {
    ...
    return (<SellerLayout title="Analytics" description="Track key inventory and product mix metrics for your seller account.">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="rounded-lg border border-gray-3">
          ...
          <div className="divide-y divide-gray-3">
            {Object.entries(metrics.categoryMap).length === 0 ? (<p className="p-5 text-custom-sm">No category data yet.</p>) : (Object.entries(metrics.categoryMap).map(([category, count]) => (<div key={category} className="px-5 py-4 flex items-center justify-between">
        ...
        <div className="rounded-lg border border-gray-3">
          ...
          <div className="divide-y divide-gray-3">
            {Object.entries(metrics.statusMap).length === 0 ? (<p className="p-5 text-custom-sm">No status data yet.</p>) : (Object.entries(metrics.statusMap).map(([status, count]) => (<div key={status} className="px-5 py-4 flex items-center justify-between capitalize">
