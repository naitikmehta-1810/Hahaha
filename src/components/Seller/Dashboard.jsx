...
const Dashboard = () => {
    ...
    const recentProducts = [...products]
        ...
        .slice(0, 5);
    return (<SellerLayout title="Dashboard" description="Overview of your store health, product inventory, and recent activity.">
      ...
      <div className="rounded-lg border border-gray-3">
        <div className="flex items-center justify-between border-b border-gray-3 px-5 py-4">
          ...
          <Link href="/seller/add-product" className="inline-flex rounded-md bg-blue px-4 py-2 text-white text-custom-sm hover:bg-blue-dark ease-out duration-200">
            Add New Product
          </Link>
        </div>
        <div className="divide-y divide-gray-3">
          {recentProducts.length === 0 ? (<p className="p-5 text-custom-sm">No products found. Add your first one.</p>) : (recentProducts.map((product) => (<div key={product.id} className="px-5 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div>
