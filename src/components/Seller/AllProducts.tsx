...
const AllProducts = () => {
  ...
  return (
    <SellerLayout
      title="All Products"
      description="Manage all your listings from a single table."
    >
      <div className="flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between mb-6">
        <input
          ...
          className="w-full sm:max-w-[340px] rounded-md border border-gray-3 bg-gray-1 px-4 py-2.5 outline-none duration-200 focus:border-transparent focus:shadow-input focus:ring-2 focus:ring-blue/20"
        />
        <Link
          href="/seller/add-product"
          className="inline-flex justify-center rounded-md bg-blue px-5 py-2.5 text-white hover:bg-blue-dark ease-out duration-200"
        >
          Add Product
        </Link>
      </div>
      ...
      <div className="w-full overflow-x-auto rounded-lg border border-gray-3">
        <div className="min-w-[840px]">
          ...
          {filteredProducts.length === 0 ? (
            <p className="px-5 py-6 text-custom-sm">No products match your search.</p>
          ) : (
            filteredProducts.map((product) => (
              <div
                key={product.id}
                className="grid grid-cols-12 gap-4 px-5 py-4 border-b last:border-b-0 border-gray-3 items-center"
              >
                ...
                <div className="col-span-2 text-right">
                  <button
                    ...
                    className="rounded-md bg-red text-white px-3 py-1.5 text-custom-xs hover:bg-red-dark ease-out duration-200"
                  >
                    Delete
                  </button>
                </div>
