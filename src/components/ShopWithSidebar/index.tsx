...
const ShopWithSidebar = () => {
  ...
  const categoryMap = useMemo(() => {
    ...
    for (const product of products) {
      map.set(product.category ?? "Uncategorized", (map.get(product.category ?? "Uncategorized") ?? 0) + 1);
    }
    return map;
  }, [products]);
            ...
            <aside className="max-w-[270px] w-full hidden xl:block">
              <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                ...
                <button
                  ...
                  className={`w-full text-left mb-2 ${selectedCategory === "all" ? "text-blue" : "text-dark"}`}
                >
                  All ({products.length})
                </button>
                {Array.from(categoryMap.entries()).map(([category, count]) => (
                  <button
                    ...
                    className={`w-full text-left mb-2 ${selectedCategory === category ? "text-blue" : "text-dark"}`}
                  >
                    {category} ({count})
                  </button>
                ))}
