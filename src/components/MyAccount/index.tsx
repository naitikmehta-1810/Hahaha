...
const MyAccount = () => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        if (!response.ok) {
          setErrorMessage(data.error ?? "Failed to load account details.");
          return;
        }
  ...
  const handleProfileSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = (await response.json()) as { message?: string; error?: string; fullName?: string };
      if (!response.ok) {
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex flex-col xl:flex-row gap-7.5">
            <div className="xl:max-w-[320px] w-full bg-white rounded-xl shadow-1 p-6">
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className={`rounded-md py-3 px-4 text-left ${activeTab === "dashboard" ? "bg-blue text-white" : "bg-gray-1 text-dark"}`}
                >
                  Dashboard
                </button>
                <button
                  onClick={() => setActiveTab("orders")}
                  className={`rounded-md py-3 px-4 text-left ${activeTab === "orders" ? "bg-blue text-white" : "bg-gray-1 text-dark"}`}
                >
                  Orders
                </button>
                <button
                  onClick={() => setActiveTab("account-details")}
                  className={`rounded-md py-3 px-4 text-left ${activeTab === "account-details" ? "bg-blue text-white" : "bg-gray-1 text-dark"}`}
                >
                  Account Details
                </button>
              </div>
            ...
            <div className="xl:max-w-[820px] w-full">
              {errorMessage && (
                <div className="mb-4 rounded-md bg-red/10 px-4 py-3 text-red">{errorMessage}</div>
              )}
                  ...
                  <form onSubmit={handleProfileSubmit} className="bg-white rounded-xl shadow-1 p-6 sm:p-8">
                    ...
                    <div className="mb-5">
                      ...
                      <input
                        ...
                        value={email}
                        disabled
                        className="rounded-md border border-gray-3 bg-gray-1 w-full py-2.5 px-5 outline-none text-dark-4"
                      />
                    </div>
                    <button
                      type="submit"
                      disabled={isLoading}
                      className="inline-flex font-medium text-white bg-blue py-3 px-7 rounded-md hover:bg-blue-dark ease-out duration-200"
                    >
                      Save Changes
                    </button>
                  </form>
